...
| `AttackWhileBowDrawHeuristic` | `attack-while-bow-draw` | kill-aura / bow-aura | Sustained melee attacks while a bow/crossbow is being drawn — the draw occupies the hand (a click would release the shot), so a melee run during one continuous draw is impossible; ships at `0` (observe) | all |
