...
| `AttackWhileInventoryOpenHeuristic` | `attack-while-inventory` | kill-aura / inventory-aura | Sustained attacks while a container GUI is open — vanilla routes the mouse to the screen, so attacking entities through an open inventory is impossible; distinct from inventory-*rotations*; ships at `0` (observe) | all |
