...
| `BowAimAccuracyHeuristic` | `bow-aim-accuracy` | bow-aura / projectile-aimbot | Arrows hit players at a super-human share over a window of shots — covers bow combat, where the melee-gated rotation suite is dark; the projectile analog of `attack-accuracy` (sustained); ships at `0` (observe) | all |
