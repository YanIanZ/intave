...
| `AttackThroughWallHeuristic` | `attack-through-wall` | wallbang kill-aura | Attacks an entity behind a solid block. `AttackRaytrace` proves angle/reach but ignores terrain; this ray-traces the target twice from the validated eye — **ignoring blocks** (must be a valid hit-box hit, so a plain miss is excluded) and **respecting blocks** (the same ray is stopped by terrain, `reach == 10`). The two disagreeing = a block between player and target along the exact ray they hit. Stable-target + post-teleport gated against desync, sustained-gated against block-update/lag; ships at `0` (observe) | all |
